...
int main(int argc, char *argv[])
{
    ...
    if (sigaction(SIGINT, &sa, NULL) != 0) {
        fatal_error("Failed to install SIGINT handler: %s", strerror(errno));
    }

    /* Also handle SIGTERM for better macOS compatibility */
    if (sigaction(SIGTERM, &sa, NULL) != 0) {
        fatal_error("Failed to install SIGTERM handler: %s", strerror(errno));
    }
